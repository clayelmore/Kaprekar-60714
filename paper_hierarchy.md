...
**Lemma 2.4 (universality criterion).** *(Proven.) Let $T$ be the map $X \mapsto \mathrm{sort}_\downarrow K(X)$ on the set of* all *multisets of $d$ digits — a finite set on which $T$ is a genuine self-map, every repdigit (including $0$) mapping to $0$ and $0$ fixed. Admissibility restricts only the starting states; orbits may pass through inadmissible multisets. Let $F \neq 0$ be a fixed multiset of $T$ (the fixed integer is $|\sum_i c_i f_i|$, whose digit multiset is $F$). Then $F$ is universal — reached by the orbit of every admissible input — if and only if no admissible orbit reaches a fixed multiset other than $F$ (in particular none reaches $0$) and no admissible orbit enters a periodic orbit of period $\geq 2$.*

*Proof.* $T$ is a self-map of a finite set, so every orbit is eventually periodic: it ends in a fixed multiset or a period-$\geq 2$ cycle, and that terminal set traps it. If $F$ is universal, an admissible orbit reaching a different fixed multiset or entering a cycle would be trapped away from $F$ — a contradiction; so neither occurs. Conversely, each admissible orbit ends in some terminal set, which by hypothesis can only be $\{F\}$. $\blacksquare$

Defining $T$ on the full multiset space is not a formality: orbits genuinely leave the admissible set in passing. Under the strict convention an admissible input can map to a near-repdigit — the ladder rule of Section 4 sends $\{9,9,9,9,9,8,7\}$ to $900000$ at $d = 7$, the excursions Appendix E.4 tracks — and some rules send fully generic inputs to nonzero repdigits, which then collapse to $0$: at $d = 3$ the multiset $\{9,6,2\}$ admits $962 - 629 = 333$. (Amusingly, this is impossible at $d = 4$ and $d = 5$ — every image is a multiple of $9$, forcing the repdigit $10^d - 1$, which exceeds the largest possible difference — and possible again at $d = 6$: $696329 - 362996 = 333333$; Computed.) The criterion therefore evaluates reachability in the full space. It makes universality decidable by finite search and splits its failure into three modes — escape to $0$, a competing fixed point, or a cycle — all of which recur below; Section 5 exhibits a rule with two fixed points and a rule with one fixed point plus cycles, and the escape mode, empty in the $d \leq 6$ censuses of Section 3, is exactly the escape class that reappears for the zero-padding chain at every $d \geq 7$ (Section 4).
...
- **L5 (transcendent).** $F$, or an explicitly described family, is universal — or universal off an explicitly characterized escape class collapsing to $0$ — at infinitely many lengths under a fixed lifting recipe. Transcendence is *orthogonal* to the L3/L4 rank distinction, and the two liftings sit on opposite sides of it: the proven transcendent object of Section 4, $60714$, is degenerate ($\mathrm{sv}_F < d$) and universal off an escape class above its roots, while the conjecturally transcendent duplication family of Section 5 is full ($\mathrm{sv}_F = d$). In the two chains this paper studies, which side a fixed point falls on is what decides whether its transcendence comes with a proof.
...
This was verified directly for this paper by intersecting the full $d=5$ and $d=6$ censuses. The selection is the right way to meet $60714$: it is not chosen for its digits, it is the unique survivor of an exhaustive test. The next section shows the survivor goes on forever — universal at its roots, universal off an exactly characterized escape class above them — and why.
...
**Computed 6.7 (coherence discriminates survival).** *Every non-repdigit state becomes coherent within $3$ steps at $k = 2$ (all $24{,}300$ multisets) and within $5$ steps at $k = 4$ (all $2{,}042{,}965$); a $300{,}000$-multiset random sample at $k = 5$, $d = 20$ enters within $4$ steps with no exceptions. At $k = 3$ — the failing fold — coherence is not reached from everywhere: the multiset $\{9^8, 5, 4^3\}$ falls into the multiset $\{6, 5^2, 4^5, 0^4\}$, a second fixed multiset of the $3$-fold rule (the fixed integer is $5{,}400{,}446{,}445$), incoherent with slice signs $(+, +, -)$. The fold's failure mode is an incoherent fixed point.*
...
The recursion between levels is syntactic. Writing $V_{j}$ for the level-$j$ form ($d_j = 4 \cdot 2^j$), the level above evaluates the level below on the two parity slices:
$$V_{j+1}(t) \;=\; 10^{\,d_j}\, V_j(t_{\mathrm{even}}) + V_j(t_{\mathrm{odd}}),$$
and the recursion is strong enough to decide signs outright — the first dividend of self-similarity.

**Lemma 6.9 (sign and plateau).** *(Proven.) (a) For every $j \geq 0$ and sorted $x$ of length $d_j$: $V_j(x) < 0$ if and only if $x_{2^j} > x_{2^{j+1}}$; $V_j(x) = 0$ if and only if $x$ is constant; $V_j(x) > 0$ otherwise. Moreover $\max |V_j| \leq 10^{d_j} - 1$. (b) Consequently a state $x$ at level $j+1$, with $a = 2^{j+1}$, $b = 2^{j+2}$, is incoherent — slice values of strictly opposite sign — exactly when it carries a critical* plateau: *either $x_a = x_{a+1} = \cdots = x_b > x_{b+1}$ with the even slice non-constant, or $x_a > x_{a+1} = \cdots = x_{b+1}$ with the odd slice non-constant.*

*Proof.* (a) Induction on $j$. At $j = 0$, $V_0 = 9(x_0 - x_3) - 900(x_1 - x_2)$: if $x_1 > x_2$ then $V_0 \leq 81 - 900 < 0$; if $x_1 = x_2$ then $V_0 = 9(x_0 - x_3) \geq 0$, vanishing exactly on constants; and $\max|V_0| = 8019 \leq 10^4 - 1$. For the step, $\max|V_{j+1}| \leq \max|V_j|\,(10^{d_j} + 1) \leq (10^{d_j} - 1)(10^{d_j} + 1) = 10^{d_{j+1}} - 1$; and since the inductive hypothesis already gives $\max|V_j| \leq 10^{d_j} - 1$, whenever $V_j(x_{\mathrm{even}}) \neq 0$ the leading term $10^{d_j} V_j(x_{\mathrm{even}})$ strictly exceeds $|V_j(x_{\mathrm{odd}})|$ in magnitude and $V_{j+1}$ takes its sign. The even slice $y_k = x_{2k}$ has $y_{2^j} = x_{2^{j+1}}$ and $y_{2^{j+1}} = x_{2^{j+2}}$, so the inductive rule transfers verbatim. If instead $V_j(y) = 0$, then $y$ is constant, sortedness squeezes every odd rank between equal even neighbours — $x_0 = x_1 = \cdots = x_{d_{j+1}-2}$ — and $V_{j+1} = V_j(x_{\mathrm{odd}})$ with odd slice $(x_0, \dots, x_0, x_{d_{j+1}-1})$, whose critical pair is interior and equal: nonnegative, zero exactly on constants. (b) Apply (a) to each slice: the even slice's critical pair is $(x_a, x_b)$, the odd's is $(x_{a+1}, x_{b+1})$, and sortedness collapses the interval between equal ranks. $\blacksquare$

Mechanical confirmation found no violations over all sorted states at levels $0, 1, 2$ ($715$; $24{,}310$; $2{,}042{,}975$) and $120{,}000$ adversarial states at level $3$ (Computed). The sign of a $2^{j+2}$-digit folded form is a comparison of two digits, and incoherence is a single shape: a run of $2^{j+1} + 1$ equal digits in the second quarter of the sorted string, with a strict drop at one prescribed end. That shape is what bounded entry must destroy — and at the two enumerable levels it is destroyed with a proof rather than a sweep.

**Proposition 6.10 (coherence entry, bound three).** *(Proven for $j = 1, 2$.) At tower levels one and two, every non-repdigit state becomes coherent within three steps, meeting no repdigit on the way. The bound is sharp at both levels.*

*Proof.* By Lemma 6.9(b) an incoherent state is a critical plateau, and on a plateau the slice values collapse to closed forms in the digit gaps alone. At level one, type A has $V_0(\mathrm{even}) = 9P$ with $P = x_0 - x_6 \geq 1$ — the equal critical pair kills the $900$-term — and $V_0(\mathrm{odd}) = 9R - 900Q$ with $Q = x_3 - x_5 \geq 1$ and $R = x_1 - x_7 \in [Q, 9]$, so the image value is $9P \cdot 10^4 - 9(100Q - R)$, a function of three gaps; type B mirrors it. The images form an $810$-member family indexed by gaps, not by states; direct computation shows every member coherent or becoming so within two further steps ($744$ immediately, $62$ in one, $4$ in two). At level two the same collapse — Lemma 6.9(a) applied to the eight-digit slices and their four-digit sub-slices — writes both slice values in seven gaps; the formulas agree with all $251{,}940$ incoherent states of the full census without exception, and the abstract family of $2 \times 1{,}247{,}400$ members is coherent within two further steps in every case. No image is ever a repdigit: images are positive multiples of $9$; a $d$-digit repdigit $r \cdots r$ has digit sum $rd$, and the tower lengths are powers of two, so $\gcd(d, 9) = 1$ and $rd \equiv 0 \pmod 9$ forces $r = 9$, i.e. $10^d - 1$; and every family value is below $8.1 \cdot 10^{d-1}$. Sharpness: the full enumerations attain entry time three at both levels. $\blacksquare$

*(Evidence at $j = 3$: sixty thousand incoherent $d = 32$ states constructed directly from the plateau shape all enter coherence within three steps — the same bound. We conjecture entry within three steps at every level.)*

What the quotient is for is excluding cycles, and one failure mode needs more care than Computed 6.8 gives it. The funnel there is verified on the pairs whose orbits never exit coherence; but nothing yet excludes a cycle that weaves through coherent and incoherent states alternately — its coherent arcs leave the partial pair system before returning, so a funnel over never-exiting pairs cannot see it, and bounded entry alone does not kill it. As stated in earlier drafts of this problem, then, the two verified properties do not imply the conjecture. The repair is to quotient by the *return map*. A coherent state's image multiset is the disjoint union of its blocks' digits — a function of the pair alone — so the entire future of a coherent state is determined by its pair. Let $\widehat{T}$ send a pair to the pair of the first coherent state on the forward orbit of its assembled image; bounded entry makes $\widehat{T}$ total.

**Theorem 6.11 (return-map reduction).** *(Proven.) Fix a tower level with folded fixed point $F$, the level below's fixed value $W$ written twice. Suppose* (E$'$)*: every non-repdigit state becomes coherent within a bounded number of steps, meeting no repdigit en route; and* ($\widehat{\mathrm{F}}$)*: $\widehat{T}$ is total on the reachable coherent pairs with no repdigit met inside any excursion, $(W, W)$ is its unique fixed pair, and it has no cycle of period $\geq 2$. Then the tower element is universal.*

*Proof.* The state space is finite, so an admissible orbit ends in a periodic set $C$. The orbit stays off repdigits and $0$ forever: before first coherence by (E$'$), and excursion by excursion afterwards by ($\widehat{\mathrm{F}}$) — noting that an excursion could also *end* at a repdigit, which is vacuously coherent with pair $(0,0)$, but $(0,0)$ would then be a reachable fixed pair distinct from $(W, W)$, excluded by uniqueness. So $C$ consists of non-repdigit states, and by (E$'$) it contains a coherent state $s^{\ast}$ with pair $p^{\ast}$. The coherent states met in order around the cycle form a $\widehat{T}$-orbit returning to $p^{\ast}$: a $\widehat{T}$-cycle, which by ($\widehat{\mathrm{F}}$) can only be the fixed pair $(W, W)$. The state following $s^{\ast}$ is then the assembled fixed point $F$, so $F \in C$, and $F$ being fixed forces $C = \{F\}$. Every admissible orbit ends at $F$. $\blacksquare$

**Computed 6.12 (the return-map funnel, complete at both levels).** *At level one, $\widehat{T}$ is total on all $1{,}464$ coherent pairs — the $262$ whose orbits exit coherence included — with no repdigit excursions, unique fixed pair $(1746, 1746)$, and no cycles. At level two the same holds on all $235{,}212$ pairs, with unique fixed pair $(17461746, 17461746)$. Together with Proposition 6.10 this discharges both hypotheses of Theorem 6.11 at $j = 1, 2$: a complete quotient-route proof of universality at both levels, independent of the basin enumerations that first established it.*

This is how the analogy that has run through the paper closes. The classical four-digit map reduces to a $54$-state gap system funnelling to $(6,2)$ (Section 7); the coherent fold dynamics reduces, one level up, to the $1{,}464$-pair return map funnelling to $(1746, 1746)$; the level above, to $235{,}212$ pairs funnelling to the fixed pair doubled. The boundary between pairs that keep coherence and pairs that lose it — which no tested inequality or congruence describes — no longer needs describing: the return map absorbs the excursions, and the certificate-cost shadow of Section 7 falls where it belongs, on the funnel property of $\widehat{T}$ itself. Theorem 6.11 converts Conjecture 6.6 into exactly two uniformities: bounded coherence entry at every level (proven at $j = 1, 2$ with sharp bound three, the same bound holding in every level-three test), and the funnel property of every level's return map (complete at $j = 1, 2$). Both are open above — and not for lack of a method so much as a measured obstruction: the entry proofs are finite certificates in the digit gaps, and the one reduction that would keep such a certificate bounded across levels — dependence on the leading gap-windows only — fails by measurement: image coherence at level two depends on the low-order gaps too ($921$ of $1{,}710$ coarse classes mix outcomes), so the family this style of certificate must check grows without bound in the level. A uniform entry lemma needs a genuine carry-and-block argument, one level above Lemma 4.5. Section 7 states the sharpened problem.
...
It is worth closing the circle with Section 4. The zero-padding proof never confronts any of this, because it never proves convergence at a fixed length at all: it proves a *reduction between lengths* (Lemma 4.3) and pays a bounded, $d$-independent cost to reach the reducing set (Lemma 4.5). The duplication chain now has both pieces, and in better than verified form: the reduction is a theorem valid at every level (Theorem 6.11), and the reaching is proven at the two enumerable levels with sharp bound three (Proposition 6.10). What blocks the ascent is measured exactly — the reaching proof is a finite gap-certificate whose size grows with the level, and the funnel half remains a computation. The precise frontier, sharpened by Section 6's findings:

**Open Problem 7.2.** *For the dyadic tower of the $1746$ rule, prove uniformly in the level $j$ the two hypotheses of Theorem 6.11, both established at $j = 1, 2$:* (E$'$) *every non-repdigit state becomes coherent within a bounded number of steps, meeting no repdigit en route — proven there with sharp bound three (Proposition 6.10), and conjectured to hold with bound three at every level; and* ($\widehat{\mathrm{F}}$) *the return map $\widehat{T}$ funnels — unique fixed pair, no cycles (Computed 6.12). By Theorem 6.11 the two together are Conjecture 6.6. For* (E$'$)*, Lemma 6.9 reduces the task to showing that the image of a critical plateau cannot sustain critical plateaus for more than two further steps; the gap-certificate that proves this at $j \leq 2$ provably cannot climb, and what is needed is a carry-and-block argument one level above Lemma 4.5. For* ($\widehat{\mathrm{F}}$)*, the pair systems outgrow enumeration at $j = 3$ and the funnel boundary admits no simple description; any proof must exploit the syntactic recursion $V_{j+1} = 10^{d_j} V_j \oplus V_j$.*
...
*The $d = 15, 16$ rows confirm one-step closure where E.1 applies and just below the even threshold; exhaustive one-step confirmations at $d = 17$ ($3{,}124{,}450$ multisets), $d = 18$ ($4{,}686{,}725$), and $d = 20$ ($10{,}014{,}905$) are in the project log [S], and the $d = 18$ confirmation — the even threshold itself — was re-run in full for this paper: all $4{,}686{,}725$ admissible multisets enter $T_{18}$ in one step, zero violations (Computed).*
...
*Proof of the recovery claim.* The admissible Case-2 inputs at $d$ have sorted form $(v^{d-3}, w, 0, 0)$ with $v \neq 0$, $w \neq v$ — eighty-one $(v, w)$ pairs — and project to the near-repdigit $\{v^{d-3}, w\}$ at length $d-2$. On a near-repdigit exactly one adjacent gap of the sorted string is nonzero — at the top rank if $w > v$, at the bottom if $w < v$ — so, writing the value in the gap form $\sum_j C_j g_j$ with $C_j$ the coefficient partial sums, the image is a single partial-sum multiple of that gap, in closed form on either ladder.*

*If $w < v$, the nonzero gap sits at the bottom rank: the core partial sums all cancel and only the final appended pair fires, $K^{(d-2)} = 9(v-w)\cdot 10^{d-4}$. For $v - w \geq 2$ this is two distinct nonzero digits above $d - 4$ zeros — admissible at $d-2$, so the orbit recovers in one step. For $v - w = 1$ it is $9 \cdot 10^{d-4}$, padding to the near-repdigit $(9, 0, \dots, 0)$; one further step gives $K^{(d-2)} = 9 \cdot 9900 = 89{,}100$, padding to $(9, 8, 1, 0, \dots, 0)$ — admissible. Recovery in at most two steps.*

*If $w > v$, the nonzero gap is at the top rank and only the leading partial sum — the coefficient $c_0 = 9900$ itself — fires: $K^{(d-2)} = 9900\,(w-v)$ on both ladders. Equivalently, $w$ carries $c_0$ while every $v$ carries the rest, and $9 + 90 - 9000 + 99000 - 99999 = -9900$ collects the even root's $v$-coefficients to the identical value. This runs through $9900, 19800, \dots, 79200$ as $w - v = 1, \dots, 8$, each admissible at $d-2$ (top digits $9,9$ when $w-v=1$, otherwise three distinct nonzero digits) above $d-4$ zeros. Recovery in one step.*

*Only the ladder roots $d - 2 \in \{5, 6\}$, lacking the appended-pair structure, fall outside the closed forms; there all eighty-one near-repdigit multisets were checked directly to reach $60714$, in at most $29$ steps at $d-2 = 5$ and $15$ at $d-2 = 6$ (Computed — a separate check, since the censuses exclude near-repdigits and so cannot be cited for them). The closed forms hold at every length, so the induction is closed uniformly in $d$; direct enumeration confirms them — seventy-two pairs recovering in one step, the nine with $v - w = 1$ in two — together with total reaching times at most $27$ steps on the odd ladder and $15$ on the even, for every $7 \leq d \leq 401$. $\blacksquare$
...
| 8 | even | `gheadcbf` | `hgfdeacb` |
| 9 | odd | `hifgadcbe` | `ihgfdeacb` |
| 10 | even | `ijgheadcbf` | `jihgfdeacb` |
| 11 | odd | `jkhifgadcbe` | `kjihgfdeacb` |
| 12 | even | `klijgheadcbf` | `lkjihgfdeacb` |
| 13 | odd | `lmjkhifgadcbe` | `mlkjihgfdeacb` |
| 14 | even | `mnklijgheadcbf` | `nmlkjihgfdeacb` |
| 15 | odd | `nolmjkhifgadcbe` | `onmlkjihgfdeacb` |
| 16 | even | `opmnklijgheadcbf` | `ponmlkjihgfdeacb` |
| 17 | odd | `pqnolmjkhifgadcbe` | `qponmlkjihgfdeacb` |
| 18 | even | `qropmnklijgheadcbf` | `rqponmlkjihgfdeacb` |
| 19 | odd | `rspqnolmjkhifgadcbe` | `srqponmlkjihgfdeacb` |
| 20 | even | `stqropmnklijgheadcbf` | `tsrqponmlkjihgfdeacb` |

Each rung extends the rung two above by one prepended letter-pair, encoding the appended zero-sum pair $(+9 \cdot 10^{d-2},\ -9 \cdot 10^{d-2})$ at the two new bottom ranks — the same signs on both ladders, the lower-rank letter leading in $\pi$ and trailing in $\sigma$ — and the four coefficients $(9900, 9, 90, -9000)$ at $60714$'s nonzero digits recur verbatim in every row. Every row was machine-checked to expand to exactly the coefficient vector of Section 4's ladder (Computed); an earlier draft of this table carried the even rungs with the pair's letters crossed between $\pi$ and $\sigma$, which flips every appended sign and yields a genuinely different map — one that still fixes $60714$, since the appended coefficients sit on its zeros, but whose escape class and reaching times disagree with Section 4's ($|E_8| = 45$ in place of $137$), a reminder that the fixed-point equation certifies nothing about a ladder beyond the fixed point. Under the fixed-point assignment $a = 7$, $b = 6$, $c = 4$, $d = 1$, all later letters zero, every row evaluates to $\pi \cdot s = 71460$ and $\sigma \cdot s = 10746$, so $K = |71460 - 10746| = 60714$ (machine-checked).

The recipe is mechanical at any length. At $d = 100$ (even ladder): positions $0$–$3$ carry the locked coefficients $(9900, 9, 90, -9000)$, positions $4$–$5$ the split pair $(99000, -99999)$, and positions $6$–$99$ carry forty-seven appended zero-sum pairs $(+9 \cdot 10^{2k+6},\ -9 \cdot 10^{2k+6})$ at ranks $(2k+6,\, 2k+7)$. Every coefficient from position $4$ on multiplies a zero digit of the padded fixed point, and
