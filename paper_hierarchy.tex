...
\textbf{Open Problem 7.2.} \emph{Exhibit, for some universal base rule \(c\) and some \(k \geq 2\), a set \(\mathcal{A}_m \supseteq \{k\text{-fold states}\}\) of multisets at length \(k d_0\), closed under \(c^{\otimes k}\), reached from every admissible input in boundedly many steps, on which the dynamics provably reduces to a system solved at the base --- without using zero digits. Conjecture 6.6 would follow for the corresponding tower.}
