...
Reproduces the classification theorems (Theorems 3.1-3.4) of the paper:
    d=3: 0 universal full-variable fixed points (12 rules tested)
    d=4: 4 universal fixed points, 8 rules (216 rules tested)
    d=5: 33 universal fixed points, 66 rules (5,280 rules tested)
    d=6: 506 universal fixed points, 1,174 rules (190,800 rules tested)
...
The script uses a fast sampling heuristic to identify candidate fixed points before
...
def classify(d):
    """Exhaustively classify universal full-variable fixed points at digit length d.
    ...
    print(f"  Universal full-variable fixed points: {len(fp_to_rules)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=("Classify universal full-variable fixed points at digit length d. "
                     ...
                     "the cross-verification of the d=6 count of 506 fixed points)."))
    ...
    if save_txt:
        ...
        with open(out_path, 'w') as f:
            ...
            f.write(f"# Total: {len(sorted_fps)} fixed points, {sum(len(r) for r in results.values())} universal rules\n")
            f.write(f"#\n")
            ...
            for nz in sorted(by_zeros):
                f.write(f"#   {nz} zeros: {by_zeros[nz]} fixed points\n")
